...
@pytest.fixture
def app_instance():
    """Provides a RocketlaneApp instance for tests."""
...
def test_universal_mcp_rocketlane_app_initialization(app_instance):
    """
    Test that the RocketlaneApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "rocketlane", "RocketlaneApp instance has unexpected name."


def test_universal_mcp_rocketlane_tool_docstrings_format(app_instance):
    """
    Test that each tool method in RocketlaneApp has a well-formatted docstring,
...
def test_universal_mcp_rocketlane_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in RocketlaneApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
